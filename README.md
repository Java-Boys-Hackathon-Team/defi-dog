...
**Ограничение сервера на ARM.** На одноплатнике (`aarch64`) из двух анализаторов
работает только **mythril**: проверено запуском на пробном контракте, возвращает
корректный отчёт. **Slither** запускается, но падает при компиляции - он тянет
`solc` через `solc-select`, а тот публикует сборки только под `x86-64`, и
контейнер отвечает `Exec format error`. Запустить x86-образ через эмуляцию на
этом узле нельзя: ядро Rockchip собрано без `CONFIG_BINFMT_MISC`, поэтому
`binfmt` не регистрируется. Рабочие пути, если slither нужен: собрать `solc`
под `aarch64` из исходников и положить в образ, либо выносить анализ на машину с
x86-64. На машине разработчика с Intel/AMD оба анализатора работают как прежде.
